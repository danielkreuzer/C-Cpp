...
void graph_topsort(graph * self){
    assert(self);
    assert(!graph_empty(self));

    size_t count = 0;
    int * index = (int *)malloc(sizeof(int) * (self->count));
    assert(index);
    int * flag = (int *)malloc(sizeof(int) * (self->count));
    assert(flag);
    struct matrix_graph * ptr = self->head;
    ...
    for(size_t i = 0; i < self->count; i++){
        for(size_t j = 0; j < self->count; j++){
            int temp = 0;
            if (ptr->matrix[j * self->count + i] == true){
                temp = 1;
            }
            index[i] += temp;
        }
    ...
    while(count < self->count){
        for(size_t k = 0; k < self->count; k++){
            if((index[k] == 0) && (flag[k] == 0)){
                printf("%s ", ptr->payload[k].value);
                flag[k] = 1;
            }
            for(size_t i = 0; i < self->count; i++){
                if (ptr->matrix[i * self->count + k] == true) {
                    index[k]--;
                }
            }
        }
